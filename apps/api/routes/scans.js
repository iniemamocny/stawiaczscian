...
router.head('/:id/info', async (req, res) => {
  try {
    const { id } = req.params;
    if (!/^[0-9a-f-]{36}$/.test(id)) {
...
router.head('/:id/room.glb', async (req, res) => {
  try {
...
router.get('/:id/room.glb', async (req, res) => {
  try {
    const result = await prepareGlbResponse(req.params.id, req, res);
    if (result) {
...
export { setupWebSocket, shutdownScans };
